...
- Next: Notebook visualization (02_alignment_fitting.ipynb), LandXML I/O, tag v0.2.0
...
CLI subcommand ใช้คำเต็มที่สื่อความหมายชัดเจน:
- `station-to-coord`  = station (+offset) → N, E
- `coord-to-station`  = N, E → station, offset
- `cross-check`       = field points → sta, offset table
- `build`             = PI table → element table + control points
- `compare-drawing`   = elements + drawing → gap table
- `fit-radius`        = PI table + drawing → optimised R values
