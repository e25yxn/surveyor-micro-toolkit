...
- CLI: `smt station-to-coord` / `smt coord-to-station` / `smt cross-check` complete
...
- Next: smt build (PI→element table export), LandXML I/O, web stake-out app
...
1. **Claude Code วางแผนก่อน** — เขียนแผนเป็นไฟล์ `session_logs/plan_YYYYMMDD_HHMM.md`
   (ใส่วันที่และเวลาในชื่อไฟล์เสมอ) ระบุ: จะแก้ไฟล์ไหนบ้าง / แก้อะไร / มีความเสี่ยงอะไร /
   test ที่ใช้ยืนยัน / ตัวอย่าง input จริง → output ที่คาดหวังอย่างน้อย 2-3 แถว
   แล้วแสดงข้อความใน terminal ว่า "แผนบันทึกแล้วที่ <ชื่อไฟล์> — กรุณา upload ให้ Claude (แชท) ตรวจก่อนกดทางเลือก"
2. **ผู้ใช้ upload plan file ให้ Claude (แชท) รีวิว** — Claude ตรวจแผนแล้วบอกว่า
...
ถ้า Claude (แชท) ระบุให้แก้อะไรในรีวิว ต้องเขียนชัดใน prompt ที่ส่ง Claude Code
และต้องขอ session log ยืนยันว่าแก้แล้วก่อน commit
...
หลัง commit ทุกครั้งที่มีไฟล์ input จริง (CSV, JSON, หรือไฟล์ข้อมูลอื่น) ต้องรัน
smoke test ด้วยไฟล์จริงจาก `test_data/` ก่อน push เสมอ ถ้า smoke test พัง ห้าม push
แก้ให้ผ่านก่อน แล้วค่อย push

### CLI naming convention
CLI subcommand ใช้คำเต็มที่สื่อความหมายชัดเจน บอก input และ output ในชื่อเดียว:
- `station-to-coord`  = station (+offset) → N, E
- `coord-to-station`  = N, E → station, offset
- `cross-check`       = field points → sta, offset table
- `build`             = PI table → element table + control points (planned)

ห้ามใช้ตัวย่อในชื่อ subcommand เช่น fwd, inv, xc
